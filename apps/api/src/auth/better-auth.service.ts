...
const createAuth = (prisma: PrismaService, config: ConfigService) => {
  return betterAuth({
    secret: config.get('BETTER_AUTH_SECRET') as string,
    baseURL: config.get('BETTER_AUTH_URL') as string,
    trustedOrigins: ['http://localhost:3000'], //로컬 테스트용 CORS 허용
    ...
    socialProviders: {
      ...
      kakao: {
        clientId: config.get('KAKAO_CLIENT_ID') as string,
        clientSecret: config.get('KAKAO_CLIENT_SECRET') as string,
        disableDefaultScope: true,
        scope: ['profile_image'], //프로필 이미지만 가져오기
      },
      naver: {
        clientId: config.get('NAVER_CLIENT_ID') as string,
        clientSecret: config.get('NAVER_CLIENT_SECRET') as string,
      },
    },
  })
}
