...
- **ArgoCD**: Application/ApplicationSet 정의는 이 레포가 아니라 [skkuding/lab](https://github.com/skkuding/lab)의 `k8s/argocd/applications/proseed/`에 있다. 이 레포는 워크로드 manifest만 두고, 배포 대상·시점·이미지 갱신 전략은 lab에서 관리. 여기에 Application을 추가하면 lab이 관리하는 것과 같은 리소스를 두 Application이 소유하게 되어 sync 충돌이 난다
